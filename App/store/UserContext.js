...
const UserProvider = ({ children }) => {
  ...
  const getServices = async () => {
    ...
    if (user && token) {
      try {
        const responseServices = await axiosClient.get(
          `/api/getServices?gender=${user.gender}`
        );
        if (
          responseServices.data.services &&
          responseServices.data.services.length > 0
        ) {
  ...
  const getSiteInfos = async () => {
    ...
    if (user && token) {
      try {
        ...
        if (
          responseSiteInfos.data.infos &&
          responseSiteInfos.data.infos.length > 0
        ) {
  ...
  const logoutForDeleteAccount = async () => {
    await AsyncStorage.removeItem("token");
    await AsyncStorage.removeItem("user");
    setIsLogged(false);
  };
